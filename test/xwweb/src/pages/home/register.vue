				...
				<div class="register-main">
					<form action="#" method="post">
						...
						<div class="password underline">
							<div class="icon"></div>
							<input type="password" v-model="password" name="password" placeholder="设置密码(8-16个字符,数字+字符)" pattern="^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$"/>
						</div>
						...
						<div class="referrer underline">
							<input type="text" placeholder="请填写推荐人手机号" v-model="spreadUser"/>
						</div>
...
export default {
  ...
  methods: {
    ...
    getreferrer() {
      let referrerCode = this.$route.params.code || "",
        dataObj = { code: referrerCode },
        path = _path.getApiUrl("getspreaduser");
      this.$axios
